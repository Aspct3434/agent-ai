...
SYSTEM_DIRECTIVE = (
    ...
    # ── Pre-installed runtimes (do not reinstall) ─────────────────────────────
    "The standard developer runtimes are ALREADY installed system-wide in this "
    "environment and are on PATH for every user: Node.js with npm and npx, Python 3 "
    "with pip, and the Rust toolchain (rustc/cargo). A React, Vue, Vite, or Next.js "
    "app is built with the pre-installed npx/npm (e.g. 'npm create vite@latest') -- "
    "use them directly. You run as a non-root user, so do NOT waste iterations trying "
    "to 'apt-get install nodejs', 'sudo apt-get', or otherwise reinstall a runtime "
    "that get_system_environment already reports as present; that will only fail. "
    ...
    "The required runtime is almost certainly pre-installed, so verify with "
    "get_system_environment first. If a genuinely required runtime is truly "
    "unavailable and cannot be installed, say so explicitly in your answer rather "
    "than quietly shipping a different deliverable and claiming the task is done. "
    # ── Content generation ───────────────────────────────────────────────────
