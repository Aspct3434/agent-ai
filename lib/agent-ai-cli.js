...
function pythonHasVenv(command, args = []) {
  return capture(command, [...args, "-m", "venv", "--help"]).ok;
}

function pythonReady() {
  const spec = pythonCommand();
  return Boolean(spec && pythonHasVenv(spec[0], spec[1]));
}
...
function ensurePythonForLocalMode() {
  ...
  if (installPythonForLinux() && pythonReady()) return;
  const spec = pythonCommand();
  if (!spec) {
...
function pythonCommand() {
  return pythonCandidates().find(([command, args]) => pythonMeetsMinimum(command, args)) || null;
}
...
async function startLocalServices(config) {
  ...
  const pythonSpec = pythonCommand() || (config.dryRun ? ["python3", []] : null);
  if (!pythonSpec) {
