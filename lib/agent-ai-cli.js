...
function box(title, body) {
  ...
  const lines = [top];
  if (titleLine) {
    lines.push(`│ ${padRight(c.cyan(titleLine), width)} │`);
...
function banner() {
  const line1 = "    AAAAA   GGGGG  EEEEE  N   N TTTTT      AAAAA  III";
  const line2 = "   A     A G       E      NN  N   T       A     A  I ";
  const line3 = "   AAAAAAA G  GGG  EEEE   N N N   T       AAAAAAA  I ";
  const line4 = "   A     A G    G  E      N  NN   T       A     A  I ";
  const line5 = "   A     A  GGGG   EEEEE  N   N   T       A     A III";
  return [
    "",
    c.bold(hex("#8B5CF6", line1)),
    c.bold(hex("#A855F7", line2)),
    c.bold(hex("#D946EF", line3)),
    c.bold(hex("#EC4899", line4)),
    c.bold(hex("#F43F5E", line5)),
    "",
    `                  ${hex("#10B981", "◆")} ${c.bold("AGENT AI")} ${hex("#10B981", "◆")}`,
    ""
  ].join("\n");
}
...
class Prompter {
  ...
  async question(label, defaultValue = "", options = {}) {
    ...
    if (options.secret) {
      const prompt = defaultValue ? `${hex("#06B6D4", "?")} ${c.bold(label)} ${c.dim("[saved value hidden, press Enter to keep]")} ${hex("#06B6D4", "❯")} ` : `${hex("#06B6D4", "?")} ${c.bold(label)} ${hex("#06B6D4", "❯")} `;
      const ans = await this.secretQuestion(prompt, defaultValue);
      this.write(`\u001b[1A\u001b[2K${hex("#10B981", "✔")} ${c.bold(label)} ${c.dim("·")} ${hex("#10B981", "***")}\n`);
      return ans;
    }
    const suffix = defaultValue ? ` ${c.dim("[" + defaultValue + "]")}` : "";
    const prompt = `${hex("#06B6D4", "?")} ${c.bold(label)}${suffix} ${hex("#06B6D4", "❯")} `;
  ...
  async secretQuestion(prompt, defaultValue = "") {
    ...
    // Interactive terminal: echo a "*" per typed character (OpenClaw-style)
    // so the user gets visible feedback without revealing the secret.
    output.write(prompt);
    return new Promise((resolve) => {
      let value = "";
  ...
  async select(label, choices, defaultValue) {
    ...
    this.write(`${hex("#06B6D4", "?")} ${c.bold(label)}\n`);
    choices.forEach((choice, index) => {
      const marker = choice.value === defaultValue ? hex("#10B981", "❯") : " ";
      const note = choice.note ? c.dim(` (${choice.note})`) : "";
      this.write(`  ${marker} ${choice.value === defaultValue ? hex("#10B981", String(index + 1)) : String(index + 1)}) ${choice.value === defaultValue ? c.bold(choice.label) : choice.label}${note}\n`);
    });
    const prompt = `  ${hex("#06B6D4", "❯")} `;
    let answer = "";
...
function logStep(message, cwd) {
  if (cwd && cwd !== process.cwd()) {
    console.log(`${hex("#3B82F6", "◇")} ${message} ${c.dim(`(${cwd})`)}`);
  } else {
    console.log(`${hex("#3B82F6", "◇")} ${message}`);
  }
}
...
function pythonHasVenv(command, args = []) {
  ...
  try {
    probeParent = fs.mkdtempSync(path.join(os.tmpdir(), "agent-ai-venv-probe-"));
...
async function collectInstallConfig(options) {
  ...
  const ready = await prompter.confirm(c.red("Start installation now with these choices?"), false);
  if (!ready) {
...
function printInstallSummary(config) {
  console.log("");
  console.log(c.red("QuickStart setup summary"));
  console.log(box(
    "Choices",
    [
      `Install directory: ${config.installDir}`,
      `Model provider:    ${config.provider}`,
      `Model API key:     ${missingCredential(config) ? "missing" : "provided / not required"}`,
      `Sandbox mode:      ${config.sandbox}`,
      `Messaging app:     ${config.messaging}`,
      `Messaging token:   ${missingMessagingCredential(config) ? "missing" : "not required or provided"}`,
      `Start services:    ${config.noStart ? "no" : "yes"}`
    ].join("\n")
  ));
}
...
function missingMessagingCredential(config) {
  const messaging = config.messagingSettings;
  if ((config.messaging === "telegram" || config.messaging === "both") && !messaging.telegramToken) return "TELEGRAM_BOT_TOKEN";
  if ((config.messaging === "discord" || config.messaging === "both") && !messaging.discordToken) return "DISCORD_BOT_TOKEN";
  return "";
}
...
async function install(options) {
  ...
  if (missingBotToken) {
    console.log(c.yellow(`WARN ${missingBotToken} is blank. Messaging integration will not work until you add it to ${path.join(config.installDir, "an-api.env")}.`));
  }
  writeConfig({
    installDir: config.installDir,
