...
class Prompter {
  ...
  async question(label, defaultValue = "", options = {}) {
    ...
    if (options.secret) return this.secretQuestion(prompt, defaultValue);
  ...
  switch (provider) {
    ...
    case "vllm":
      return {
        ...
        openaiKey: process.env.OPENAI_API_KEY || "dummy",
...
async function collectProviderSettings(prompter, provider, mode) {
  ...
  if (mode === "quickstart") return defaults;
  ...
  if (provider === "kimi") {
    ...
    settings.moonshotBase = await prompter.question("Moonshot API base", defaults.moonshotBase);
...
async function collectMessagingSettings(prompter, messaging, mode) {
  ...
  if (mode === "quickstart" || messaging === "none") return settings;
  if (messaging === "telegram" || messaging === "both") {
    ...
    settings.telegramAllowed = await prompter.question("Telegram allowed chat IDs, blank = all", settings.telegramAllowed);
  }
  if (messaging === "discord" || messaging === "both") {
    ...
    settings.discordAllowed = await prompter.question("Discord allowed user IDs, blank = all", settings.discordAllowed);
  }
  return settings;
}
...
async function collectInstallConfig(options) {
  ...
  const mode = normalizeChoice(options.mode, ["quickstart", "manual"], "mode") || await prompter.select(
    ...
    [
      { label: "QuickStart", value: "quickstart", note: "recommended defaults, configure details later" },
