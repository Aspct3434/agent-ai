...
function printInstallSummary(config) {
  const keyStatus = missingCredential(config) ? hex("#F59E0B", "missing") : hex("#10B981", "provided");
...
async function startServices(config) {
  if (config.sandbox === "on") {
    ...
    try {
      await run("docker", ["compose", "up", "-d", "--build"], { cwd: config.installDir, dryRun: config.dryRun, stdio: config.dryRun ? "inherit" : "pipe" });
...
async function startLocalServices(config) {
  ...
  await run(venvPython, ["-m", "pip", "install", "--upgrade", "pip"], { cwd: config.installDir, dryRun: config.dryRun, stdio: config.dryRun ? "inherit" : "pipe" });
  await run(venvPython, ["-m", "pip", "install", "-r", "requirements.txt"], { cwd: config.installDir, dryRun: config.dryRun, stdio: config.dryRun ? "inherit" : "pipe" });
  spinnerDeps.succeed("Python dependencies installed");

  const spinnerUi = new Spinner("Installing control panel dependencies...").start();
  await run("npm", ["ci"], { cwd: path.join(config.installDir, "control-panel"), dryRun: config.dryRun, stdio: config.dryRun ? "inherit" : "pipe" });
...
async function startTui(config) {
  ...
  let python;
  if (fs.existsSync(venvDir)) {
    python = virtualEnvPython(venvDir);
  } else if (fs.existsSync(path.join(config.installDir, "venv"))) {
    python = virtualEnvPython(path.join(config.installDir, "venv"));
  } else {
    throw new Error("Python virtual environment not found. Please run 'npx @aspct3434/distill-agent install' first.");
  }
  
  await run(python, ["src/tui.py"], { cwd: config.installDir, stdio: "inherit" });
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  switch (command) {
    ...
    case "logs":
      ...
      break;
    case "tui":
      await startTui(resolveRuntimeConfig(options));
      break;
    default:
