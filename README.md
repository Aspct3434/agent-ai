...
src/
  ...
  tools.py          -- ToolManager: MCP servers, terminal, file, site publishing
...
**Task-contract system** — the model's first call is always `set_task_contract`, declaring whether the task needs execution evidence (files created, site published, DB mutated, process started) or just a text answer. The loop gates the final response on that evidence, so "I'll create the file now" with no actual tool call is impossible. During execute mode, the tool list is narrowed to evidence producers and `tool_choice=required` prevents prose escape.

**Session-per-FIFO-lane concurrency** — each session gets its own asyncio queue + worker task; different sessions run concurrently; same-session messages are strictly ordered.

**Skill distillation** — after a successful run, an LLM synthesizes the trajectory into a parameterized Python function saved to `skills/`. A quality gate requires ≥2 successful side-effect calls; the function is syntax-checked before writing.

**Prompt caching** — system directive + tool schemas carry `cache_control: ephemeral` breakpoints; fast/strong model tiers share the same provider so cache hits apply across escalations.
...
# Or run locally (requires Python 3.11+, bash available on PATH for Windows)
...
Commands run through `execute_terminal_command` use `bash` if available on `PATH` (Git Bash or WSL). Install [Git for Windows](https://git-scm.com/download/win) to get bash. Without bash, the agent falls back to cmd.exe with a warning — POSIX commands like `mkdir -p` will not work correctly. **Docker is the recommended runtime on Windows.**
