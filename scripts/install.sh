...
EOF
  read -r -p "Memory [1]: " value
  case "${value:-1}" in
    1|lite) printf 'lite' ;; 2|hybrid) printf 'hybrid' ;;
    *) echo "Invalid memory choice: $value" >&2; exit 2 ;;
  esac
}
...
use_hybrid="false"
if [[ "$memory" == "hybrid" ]]; then use_hybrid="true"; fi
...
generated_env() {
  ...
  env_line AGENT_MAX_TOKENS "2048"
  env_line AGENT_PLANNING_MAX_TOKENS "1024"
  env_line AGENT_ARTIFACT_MAX_TOKENS "20000"
  env_line AGENT_FINAL_MAX_TOKENS "1536"
  ...
  env_line AGENT_USE_HYBRID_MEMORY "$use_hybrid"
...
if [[ "$NO_START" -eq 0 ]]; then
  if [[ "$sandbox" == "on" ]]; then
    ...
    # so a fresh machine still gets the fast install. Fall back to plain pip if uv
    # can't be installed. The heavy ML stack (requirements-ml.txt) is only added
    # for hybrid memory mode. --index-strategy unsafe-best-match lets uv pick
    # torch's CPU wheel from the extra index declared in requirements-ml.txt.
    req_args=(-r requirements.txt)
    [[ "$memory" == "hybrid" ]] && req_args+=(-r requirements-ml.txt)
    if command -v uv >/dev/null 2>&1; then
      (cd "$ROOT_DIR" && uv pip install --index-strategy unsafe-best-match "${req_args[@]}" --python .run-venv/bin/python)
    elif (cd "$ROOT_DIR" && .run-venv/bin/python -m pip install --upgrade pip uv); then
      (cd "$ROOT_DIR" && .run-venv/bin/python -m uv pip install --index-strategy unsafe-best-match "${req_args[@]}" --python .run-venv/bin/python)
    else
